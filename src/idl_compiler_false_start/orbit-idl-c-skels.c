...
void
orbit_idl_output_c_skeletons(IDL_tree tree, OIDL_Run_Info *rinfo, OIDL_C_Info *ci)
{
  fprintf(ci->fh, "/*\n"
  ...
  ck_output_skels(tree, ci);
  ck_output_poastuff(tree, ci);
}
...
cbe_skel_do_interface(IDL_tree tree, OIDL_C_Info *ci)
{
  ...
  fprintf(ci->fh,
	  ...
	  "{(ORBit_impl_finder)&get_skel_%s, \"%s\", &%s__classid,"
	  " INIT_VEPVMAP_%s};\n",
