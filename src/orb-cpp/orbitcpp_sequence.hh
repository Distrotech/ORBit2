...
#define ORBITCPP_DECLARE_SEQUENCE(CPPType, CPPElem, CElem, CType)		\
    class CPPType: public ::_orbitcpp::UnboundedSequence<CPPElem, CElem,	\
				CPPElem##_seq_elem_traits, CType>		\
    {										\
        typedef ::_orbitcpp::UnboundedSequence<CPPElem, CElem,			\
		CPPElem##_seq_elem_traits, CType> Super;			\
										\
    public:									\
	CPPType (size_t max = 0): Super (max)					\
	    {}									\
	CPPType (size_t max, size_t length, buffer_t buffer,			\
                 CORBA::Boolean release = false):				\
            Super (max, length, buffer, release)				\
	    {}									\
	CPPType (const CPPType &other): Super (other)				\
	    {}									\
										\
    protected:									\
	    ...
	    return CType##__alloc ();						\
										...
										\
	c_value_t* alloc_c_buf (CORBA::ULong length) const {			\
	    return CType##_allocbuf (length);					\
	}									\
    };										\
										\
