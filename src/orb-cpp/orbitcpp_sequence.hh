...
namespace _orbitcpp {
	...
	void _orbitcpp_pack (c_seq_t &_c_seq) const {
	    for (index_t i = 0; i < _length; i++)
		traits_t().pack_elem (_buffer[i], _c_seq._buffer[i]);
	}
	
	// ORBit2/C++ extension: fill C++ sequence from C sequence
	void _orbitcpp_unpack (const c_seq_t &_c_seq) {
	    for (index_t i = 0; i < _length; i++)
		traits_t().unpack_elem (_buffer[i], _c_seq._buffer[i]);
	}
    };
